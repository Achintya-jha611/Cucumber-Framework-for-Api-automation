...
public class utils {
    ...
    public RequestSpecification requestSpecBuilder() throws IOException {
        ...
        req=new RequestSpecBuilder().setBaseUri(getGlobalKeys("baseUrl")).setContentType(ContentType.JSON).addFilter(RequestLoggingFilter.logRequestTo(stream)).addFilter(ResponseLoggingFilter.logResponseTo(stream)).build();
        return req;}
        return req;
    }
